...
def blog_single(request,pid):
    ...
    post.save()
    new_count = post.counted_views
    
    prev_post = Post.objects.filter(id__lt = post.id,status = 1,published_date__lte = timezone.now()).first()
    next_post = Post.objects.filter(id__gt = post.id,status = 1,published_date__lte = timezone.now()).last()
